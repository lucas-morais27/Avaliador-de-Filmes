...
class ListaAvaliacoes extends ChangeNotifier {
  ...
  List<Avaliacao> avaliacoesPorProducao(String id) => _avaliacoes.where((p) => p.producaoid == id).toList();
  List<Avaliacao> avaliacoesPorUsuario(String id) => _avaliacoes.where((p) => p.userid == id).toList();

  set avaliacoes(List<Avaliacao> avaliacoes) => _avaliacoes = avaliacoes; 

  adicionar(Avaliacao avaliacao){

    if(_avaliacoes.where(
      (element) 
  ...
  bool jaAvaliou(String idProducao, String idUsuario){
    return _avaliacoes.where((element) => element.producaoid == idProducao && element.userid == idUsuario).isNotEmpty;
  }

  editar(Avaliacao avaliacao){
    int idx =_avaliacoes.indexWhere(
      (element) 
      => element.producaoid == avaliacao.producaoid 
      && element.userid == avaliacao.userid);
    if(idx >= 0){
